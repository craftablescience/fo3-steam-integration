...
const UInt32 kNiTMapLookupAddr = 0xBEA7B0;
...
struct NiVector3
{
	...
	bool RayCastCoords(NiVector3 &posVector, NiMatrix33 &rotMatrix, float maxRange, UInt32 axis = 0);
};
...
{
	MessageBoxA(NULL, "NiTArray::AddAtIndex() is not implemented", "Error", MB_ICONWARNING);
}

template <typename T> void NiTArray<T>::SetCapacity(UInt16 newCapacity)
{
	ThisCall(0x6E2F60, this, newCapacity);
}
